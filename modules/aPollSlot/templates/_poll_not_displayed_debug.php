<?php $poll = isset($poll) ? $sf_data->getRaw('poll') : null; ?>
...
<div class="a-poll-nodisplay-help">
    ...
    <ul>
        <li><?php echo a_('Publication dates'); ?></li>
        <ul>
        ...
        <ul>
            <li><?php echo a_('Setting: %setting%', array('%setting%' => true == aPollToolkit::getPollAllowMultipleSubmissions($poll) ? a_('Multiple submissions allowed') : a_('Only one answer allowed'))); ?></li>
        </ul>
    </ul>
    
    <?php echo a_('Change these parameters in %link%',array('%link%' => link_to('poll\'s admin', '@a_poll_poll_admin_edit?id=' . $poll->getId()))); ?>
</div>